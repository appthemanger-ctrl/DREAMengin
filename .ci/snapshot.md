# Repo Snapshot

- Time: 2026-02-13 16:52:32 PST
- Repo: appthemanger-ctrl/DREAMengin
- Branch: completedream
- Commit: 2e2f8674f2e0eb4ed0e905d59064426def9a2e1b

## Key files (root)
- ✅ package.json
- ❌ pnpm-lock.yaml
- ❌ pnpm-workspace.yaml
- ❌ turbo.json
- ❌ next.config.js
- ✅ next.config.mjs
- ❌ next.config.ts

## package.json locations (maxdepth 5)
```txt
package.json
```

## Root tree (2 levels)
```txt
.ci
.ci/snapshot.diff.txt
.ci/snapshot.md
.env.example
.git
.github
.github/workflows
.gitignore
Dockerfile
Dockerfile.dev
README.md
app
app/about
app/admin
app/ads
app/analytics
app/anchor-demo
app/api
app/auth
app/connectors
app/create
app/demo
app/discover
app/dream-effects
app/dreamengin
app/edit-profile
app/error.tsx
app/feed-settings
app/gesture-nav
app/global-error.tsx
app/globals-enhanced.css
app/globals.css
app/home
app/join
app/lab
app/layout.tsx
app/login
app/messages
app/music
app/not-found.tsx
app/page.tsx
app/physics-lab
app/profile
app/settings
app/shop
components
components/AIAssistant-voice-enhanced.tsx
components/AIAssistant.tsx
components/AIAssistant.tsx.backup
components/AIAssistantEnhanced.tsx
components/AdvancedSearch.tsx
components/AnalyticsPanel.tsx
components/AnchorWidget.tsx
components/AnchorWidgetOrchestrator.tsx
components/CollaborativeCanvas.tsx
components/CommandPalette.tsx
components/ContentScheduler.tsx
components/CreatePostModal.tsx
components/DashboardLayout-enhanced.tsx
components/DashboardLayout.tsx
components/DrEamsModeToggle.tsx
components/DrEamsVoiceAssistant.tsx
components/DragToAnchorClose.tsx
components/FeedCard-enhanced.tsx
components/FeedCard.tsx
components/FloatingActionBubble.tsx
components/GestureNavigationDemo.tsx
components/HomeDashboard.tsx
components/HomeFeed.tsx
components/HomeRadialNav.tsx
components/HomeSpace.tsx
components/InnerDreams.tsx
components/InnerDreamsButton.tsx
components/LandingHero.tsx
components/LedgerChart.tsx
components/MessagesClient.tsx
components/MobileFeedCard.tsx
components/MobileFloatingActionButton.tsx
components/MobileNavBarEnhanced.tsx
components/NavBar-enhanced.tsx
components/NavBar.tsx
components/NotificationCenter.tsx
components/PhysicsLab.tsx
components/ProfileEditor.tsx
components/ProfileSpace.tsx
components/PullToRefresh.tsx
components/ShrunkMode.tsx
components/SkeletonLoaders.tsx
components/StarsBackground.tsx
components/ThemeToggle.tsx
components/ToastSystem.tsx
components/TopBar.tsx
components/WheelLayout.tsx
components/WidgetBubble.tsx
components/dreamengin
components/dreamnav
components/shaders
components/spatial
components/three
components/universe
components/v1-ui
components/widgets
docker-compose.yml
docs
docs/ADD_WORKFLOW.md
docs/GESTURE_NAVIGATION_IMPLEMENTATION.md
docs/HORIZON_FIREWALL.md
docs/WEBAPP_FINAL_FORM.md
docs/WIDGET_SYSTEM_V2.md
docs/dreamengin-ux-spec.tex
dr-eams
dr-eams/capabilities.yaml
dr-eams/tools.ts
hooks
hooks/use-spatial.ts
hooks/useHideOnScroll.ts
hooks/useTick.ts
kubernetes
kubernetes/deployment.yaml
lib
lib/adari.ts
lib/agents
lib/ai
lib/connectors
lib/dreamnav
lib/ledger-data.ts
lib/navigation
lib/supabase
lib/ui
lib/utils.ts
lib/webgpu.ts
lib/widgets
next-env.d.ts
next.config.mjs
package.json
playwright.config.ts
postcss.config.js
postcss.config.mjs
proxy.ts
public
public/IMG_3362.png
public/apple-touch-icon.png
public/boogeyman-guardian.png
public/dr-eams-torus.jpeg
public/dr-eams.jpeg
public/favicon-16.png
public/favicon-32.png
public/favicon.ico
public/favicon.png
public/icon-192.png
public/icon-512.png
public/idari-banner.jpeg
public/images
public/logo-core.png
public/logo-dark.png
public/logo-icon.jpeg
public/logo-icon.png
public/logo-small.png
public/logo-swirling.png
public/logo.png
public/manifest.json
public/videos
scripts
scripts/deploy.sh
scripts/generate-webapp-final-form.mjs
scripts/postbuild.js
scripts/postbuild.ts
scripts/setup-database.sql
styles
styles/globals.css
styles/theme.css
supabase
supabase/config.toml
supabase/migrations
supabase/schema-final.sql
supabase/seed.sql
tailwind.config.ts
tailwindcss-animate.d.ts
terraform
terraform/main.tf
tests
tests/boogieman.test.ts
tests/e2e
tests/example.spec.ts
tests/navigation
tsconfig.json
types
types/ads.ts
types/ai-system.ts
types/ai.ts
types/ccc.ts
types/experience.ts
types/marketplace.ts
types/spatial.ts
types/supabase.ts
types/widget-system-v2.ts
types/widgetConfigs.ts
types/widgets.ts
validate-deployment.js
vercel.json
vitest.config.ts
workflows
workflows/github-actions.yml
```
