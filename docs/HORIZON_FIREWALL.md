...
- **Horizon / causal boundary**: decisions happen at the edge (`middleware.ts`) before requests hit app routes.
...
- `middleware.ts` — applies headers + runs the Horizon Firewall.
- `lib/security/horizon-firewall.ts` — the firewall engine.
- Optional env var:
  - `HORIZON_LEDGER_SECRET` — enables signed client-side “ledger” cookies.
