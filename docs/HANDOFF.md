...
|---|---|---|---|---|---|
| **auto** | 2026-03-20 13:06 UTC | `470d9ee` | copilot/do-something-awesome-again | Copilot | +1 added  ~3 modified<br>feat: add DaydreamPulseStrip — live Dream Surfaces panel on HomeDream Surface — Reasoning:   A new animated 'Dream Surfaces' panel is mounted inside WorkspaceDashboard   directly below the greeting section, between the quick-action chips and   the DreamSpace portal. The strip shows all 6 canonical Daydream surfaces   (Music, Games, Lab, Code, Brand, Create) as frosted-glass orbs with   staggered CSS pulse rings, a golden sweep shimmer, per-domain color glows,   live-status dots, and real surface navigation on tap.  Architecture justification:   - docs/ARCHITECTURE.md §8  — Gold / light-blue / white premium palette;       glassmorphism (backdrop-filter blur) matches .de-glass pattern.   - docs/AXIOMS.md Axiom 4   — 'Stylized — premium, intentional, designed.'   - docs/LAW.md Product law 3 — Every visible action does something real;       each orb navigates to its canonical /daydream/* route.   - docs/ARCHITECTURE.md §10 — No runRenderLoop / setInterval; all motion       is CSS @keyframes (transform + opacity only) — battery-safe and       render-on-demand aligned.   - framer-motion entrance stagger only; no continuous JS animation timers.   - Six canonical surfaces from docs/ARCHITECTURE.md §1 Daydream Surface       Network, linked to the canonical routes from docs/ARCHITECTURE.md §2.  Performance impact: better — GPU-composited CSS keyframes replace no   prior animation; no JS timer overhead; will-change: transform/opacity   on pulse rings and sweep shimmer; framer-motion entrance only fires once.  Bug fixes included in this commit:   - ConnectorRow.tsx: repaired corrupt handleDisconnect() function that       had two merged versions (missing outer catch closing brace + duplicate       try/catch body). Turbopack parse error at EOF. Fixed to single clean       try/catch/finally. Architecture: docs/ARCHITECTURE.md §3 component layer.   - HomeRadialNav.tsx: truncated to the canonical redirect stub (29 lines).       The file had the old 1090-line legacy body still present after the       redirect re-export was prepended, causing 'Cannot redeclare exported       variable default' TypeScript error. Per docs/LAW.md §10 (repurpose       before rebuilding) and FEATURE_STATUS.md §8 item 10 (unify DreamMenu).<br>➕: `components/home/DaydreamPulseStrip.tsx`<br>✏️: `components/HomeRadialNav.tsx`, `components/connectors/ConnectorRow.tsx`, `components/home/WorkspaceDashboard.tsx` |
| **auto** | 2026-03-20 08:12 UTC | `d8cd22d` | completedream | appthemanger-ctrl | no file changes<br>Merge pull request #252 from appthemanger-ctrl/copilot/update-home-page-scrolling-box — fix: runtime regions resize with DreamDMBar — true layout reflow<br> |
| **auto** | 2026-03-20 07:13 UTC | `c53166b` | copilot/update-home-page-scrolling-box | Copilot | +1 added  ~3 modified<br>feat: constrained scrollable runtime boxes with zoom controls and in-region navigation — Co-authored-by: appthemanger-ctrl <253588904+appthemanger-ctrl@users.noreply.github.com><br>➕: `components/runtime/RuntimeShell.tsx`<br>✏️: `components/dreams/DreamsSpacePanel.tsx`, `components/home/WorkspaceDashboard.tsx`, `components/runtime/RuntimeView.tsx` |
