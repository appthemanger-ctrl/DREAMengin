...
| **auto** | 2026-04-12 01:59 UTC | `1584d9e` | copilot/fix-build-log-errors-another-one | Copilot | ~2 modified<br>feat(quantum): real quantum circuit simulator — state vector engine, QAOA/VQE, QUBO cost, live animation, wired into PortfolioEngin — Reasoning: QuantumCircuitCanvas was a static canvas stub. Replace with a complete in-browser quantum circuit simulator that executes real gate operations on a complex-amplitude state vector and returns accurate measurement probabilities to the portfolio optimizer.  Architecture justification: render-on-demand rule — docs/ARCHITECTURE.md §10. All computation is synchronous browser-side; no polling; RAF loop runs only while active=true and stops on completion or unmount.  Performance impact: better — removed continuous repaint stub; simulation runs once per active session, RAF terminates automatically.  Agent-Logs-Url: https://github.com/appthemanger-ctrl/DREAMengin/sessions/7237a24c-9194-43c1-a703-99438f92669b  Co-authored-by: appthemanger-ctrl <253588904+appthemanger-ctrl@users.noreply.github.com><br>✏️: `engins/portfolio/PortfolioEngin.tsx`, `engins/portfolio/QuantumCircuitCanvas.tsx` |
