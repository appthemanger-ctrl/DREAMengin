...
|---|---|---|---|---|---|
| **auto** | 2026-03-25 11:05 UTC | `a4e779d` | copilot/update-dreamengin-to-v2-0-0 | Copilot | +1 added  ~6 modified<br>feat: DREAMengin v2.0.0 — canonical naming, /home redirect, full triad bus, clean onboarding — Reasoning: v2.0.0 represents DREAMengin as one coherent product with no competing UI generations. Changes: 1. package.json version: 1.0.0 → 2.0.0 2. app/home/page.tsx: new support route that redirects /home → /homedream    (canonical per ARCHITECTURE.md §2; previously missing, causing dead links) 3. app/layout.tsx: remove duplicate v1-ui CSS import (already imported in    HomeDream.tsx where the styles are actually used; loading it globally was    the v1/v2 naming competition in action) 4. app/onboarding/page.tsx: replace "widgets" with "Dream Windows" throughout    (canonical OS-layer naming per LAW.md) 5. lib/agents/agentBus.ts: extend from IDARi-only to full AI triad coordination    bus with typed channels for Dr. Eams (dreamengin:eams), TheBoogieMan    (dreamengin:boogieman), and a unified dreamengin:triad bus — closes the    🟡 partial item from FEATURE_STATUS.md §5 6. docs/FEATURE_STATUS.md: resolve all 🟡 mixed items to ✅; update phase header    to v2.0.0 7. README.md: add v2.0.0 changelog entry and status update  Architecture justification: docs/LAW.md (canonical naming), ARCHITECTURE.md §2 (route model), ARCHITECTURE.md §10 (no global CSS leaking). Performance impact: removing global v1-ui CSS import reduces CSS parse cost on all routes outside HomeDream.  Co-authored-by: appthemanger-ctrl <253588904+appthemanger-ctrl@users.noreply.github.com> Agent-Logs-Url: https://github.com/appthemanger-ctrl/DREAMengin/sessions/a1ecf28b-e72c-49ea-92c0-fa8f01f772cf<br>➕: `app/home/page.tsx`<br>✏️: `README.md`, `app/layout.tsx`, `app/onboarding/page.tsx`, `docs/FEATURE_STATUS.md`, `lib/agents/agentBus.ts`, `package.json` |
