...
|---|---|---|---|---|---|
| **auto** | 2026-04-03 06:08 UTC | `6026056` | copilot/find-workaround-for-cachecomponents-break | Copilot | ~134 modified<br>feat: upgrade Babylon.js 9.0, Tailwind 4.0, enable PPR cacheComponents — - Upgrade @babylonjs/core from ^8.54.3 to ^9.1.0 - Upgrade tailwindcss from ^3.4.19 to ^4.2.2 - Add @tailwindcss/postcss, remove autoprefixer (built into TW4) - Update PostCSS config to use @tailwindcss/postcss - Migrate globals.css to @import "tailwindcss" with @config bridge - Remove content array from tailwind.config.ts (auto-detected in TW4) - Migrate 88 page routes from export const dynamic='force-dynamic' to connection() - Remove force-dynamic from 35 API routes (inherently dynamic) - Remove export const runtime='nodejs' from 3 routes (default in Next.js 16) - Enable cacheComponents: true in next.config.mjs for PPR - Update tech-foundation tests  All 2383 tests pass. Build compiles successfully with PPR enabled.  Agent-Logs-Url: https://github.com/appthemanger-ctrl/DREAMengin/sessions/c29b8b4a-7bfb-4db0-96c0-bad51d53a203  Co-authored-by: appthemanger-ctrl <253588904+appthemanger-ctrl@users.noreply.github.com><br>✏️: `app/admin/page.tsx`, `app/ads/create/page.tsx`, `app/ads/page.tsx`, `app/ads/slot/[id]/page.tsx`, `app/analytics/page.tsx`, `app/api/account/delete-data/route.ts`, `app/api/account/delete-dream/route.ts`, `app/api/account/export-data/route.ts`, `app/api/admin/ai-chat/route.ts`, `app/api/admin/child-safety/route.ts`, `app/api/admin/code-files/route.ts`, `app/api/admin/observability/route.ts` … +122 more |
