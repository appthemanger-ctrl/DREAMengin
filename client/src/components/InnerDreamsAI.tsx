...
import { X, Minus, Sparkles, Code2, Send, Terminal, Loader2, CheckCircle2, GitBranch, Server, Key } from "lucide-react";
...
export function InnerDreamsAI({
  ...
  const [innerdreamsPassword, setInnerdreamsPassword] = useState(() => localStorage.getItem('INNERDREAMS_PASSWORD') || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!innerdreamsPassword.trim()) {
      addLog('error', 'Missing Innerdreams password. Enter it in the header first.');
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className="module-header bg-slate-900/50 border-b border-white/5 cursor-grab active:cursor-grabbing flex items-center justify-between px-4 py-3 gap-3"
        onPointerDown={(e) => {
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-2 mr-1">
            ...
            <input
              type="password"
              value={innerdreamsPassword}
              onChange={(e) => {
                setInnerdreamsPassword(e.target.value);
                localStorage.setItem("INNERDREAMS_PASSWORD", e.target.value);
              }}
              placeholder="Innerdreams key"
              className="h-8 w-[180px] bg-black/30 border border-white/10 rounded-md px-2 text-xs text-slate-200 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
            />
          </div>
