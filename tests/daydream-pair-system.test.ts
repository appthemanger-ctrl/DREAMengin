...
describe('README §9 Daydream Pair System', () => {
  ...
  it('matches the canonical Daydream → Engin mapping', () => {
    const asRecord = Object.fromEntries(
      DAYDREAM_PAIR_SYSTEM.map((pair) => [pair.sideA, pair.sideB]),
    );
    expect(asRecord).toEqual(DAYDREAM_TO_ENGIN);
  });
  ...
  it('includes specialized tools and Dream Window support on every pair', () => {
    for (const pair of DAYDREAM_PAIR_SYSTEM) {
      ...
      expect(pair.enginSideMenuControl).toBe('dual-button-pill');
    }
  });
  ...
  it('uses only canonical Daydream and Engin names', () => {
    ...
    for (const pair of DAYDREAM_PAIR_SYSTEM) {
      expect(validDaydreams.has(pair.sideA)).toBe(true);
      expect(validEngins.has(pair.sideB)).toBe(true);
    }
  });
