...
describe('inferRootCause', () => {
  ...
  it('matches syntax/type compiler pattern', () => {
    const snapshot: TelemetrySnapshot = {
      ...emptySnapshot(),
      logs: [{ id: '1', timestamp: new Date().toISOString(), level: 'error', message: "engins/ContentEngin/index.tsx:1184:10 Type error: Cannot find name 'repurposeInput'." }],
    };
    const result = inferRootCause([], snapshot);
...
describe('buildFallbackPatchPlan', () => {
  ...
  it('uses immediate-action file hints when present', () => {
    ...
    const action = buildImmediateRemediationAction({
      ...rootCause,
      evidence_summary: ["engins/ContentEngin/index.tsx:1184:10 Type error: Cannot find name 'repurposeInput'."],
    });
    const plan = buildFallbackPatchPlan(rootCause, 'iter-6', action);
    expect(plan!.steps[0].file).toContain('engins/ContentEngin/index.tsx');
  });
});
...
describe('buildImmediateRemediationAction', () => {
  ...
  it('builds a syntax action from compiler evidence', () => {
    ...
    expect(action?.file_hints).toContain('engins/ContentEngin/index.tsx');
