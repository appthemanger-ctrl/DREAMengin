...
export default function HomeRadialNav({ user }: HomeRadialNavProps) {
  ...
  const callIdari = async (mode: 'bug-check' | 'update', prompt?: string): Promise<string> => {
    try {
      ...
      return `⚠️ I encountered an error communicating with IDARi: ${(e as Error)?.message || 'Unknown error'}. This might be a temporary network issue—would you like me to try again?`;
    }
  };
