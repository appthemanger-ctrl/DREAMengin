      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] mb-4 overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-2 gap-3">
            {(['vqe', 'qaoa'] as Algorithm[]).map((alg: Record<string, unknown>) => (
              <button
                key={alg}
        ...
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] mb-4 overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-2 gap-3">
            ...
            ] as { value: Backend; label: string; sub: string }[]).map((opt: Record<string, unknown>) => (
              <button
                key={opt.value}
        ...
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] mb-4 overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-2 gap-3">
            ...
            ] as { value: Ansatz; label: string; sub: string }[]).map((opt: Record<string, unknown>) => (
              <button
                key={opt.value}
                  ...
                  p > 0.02 ? (
                    <span
                      ...
                      style={{
                        ...
                        color:      i === quantumResult.probabilities.reduce(b: Record<string, unknown>, v: number, j: number => (v > quantumResult.probabilities[b] ? j : b), 0)
                          ? ACCENT
                          : `${PURPLE}aa`,
                      }}
                    >
